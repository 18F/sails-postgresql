  ...
  var adapter = {
    ...
    find: function(connectionName, table, options, cb) {
      spawnConnection(connectionName, function __FIND__(client, cb) {
        ...
        if (connectionObject.config.softDelete === true) {
          if (!options) options = {};
          if (_.has(options, 'where') && _.isObject(options.where)) {
    ...
    destroy: function(connectionName, table, options, cb) {
      spawnConnection(connectionName, function __DELETE__(client, cb) {
        ...
        var connectionObject = connections[connectionName];
        var collection = connectionObject.collections[table];
        var softDelete = connectionObject.config.softDelete;
        ...
        try {
          ...
          if (softDelete === true && !_schema[table].junctionTable) {
  ...
  function spawnConnection(connectionName, logic, cb) {
    ...
    function after(err, client, done) {
      if(err) {
        ...
        console.error('');
        console.error('Error creating a connection to Postgresql usiing the following settings:\n',connectionConfig);
        console.error('');
